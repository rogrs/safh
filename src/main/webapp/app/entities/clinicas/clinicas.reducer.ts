...
export const getSearchEntities: ICrudSearchAction<IClinicas> = (query, page, size, sort) => ({
  ...
  payload: axios.get<IClinicas>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
...
export const getEntities: ICrudGetAllAction<IClinicas> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<IClinicas>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
};
