...
export const getSearchEntities: ICrudSearchAction<IDietas> = (query, page, size, sort) => ({
  ...
  payload: axios.get<IDietas>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
...
export const getEntities: ICrudGetAllAction<IDietas> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<IDietas>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
};
