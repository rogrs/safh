...
export const getSearchEntities: ICrudSearchAction<IPacientes> = (query, page, size, sort) => ({
  ...
  payload: axios.get<IPacientes>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
...
export const getEntities: ICrudGetAllAction<IPacientes> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<IPacientes>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
};
